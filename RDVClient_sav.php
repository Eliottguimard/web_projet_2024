...
if ($db_found) {
    // Requête SQL pour récupérer les RDV
    $sql = "SELECT medecin.id, medecin.nom, medecin.prenom, listeRDV.creneau FROM listeRDV, medecin where listeRDV.client = $_SESSION[id] and medecin.id = listeRDV.medecin";
...
<!-- <?php if (!empty($RDVClient)) : ?>
    <?php foreach ($RDVClient as $RDV) : ?>
        <div class="medecin">
            <?php $horaire= creneauVersHoraire($RDV['creneau']); ?>
            <h2><?php echo htmlspecialchars("Rendez vous " . $horaire . " avec " . $RDV['nom'] . " " . $RDV['prenom']); ?>
                <button class="cta-button"><a class="calendrier" href="annulationRdv.php?medecin=<?php echo $RDV['id']?>&creneau=<?php echo $RDV['creneau']?>">Annuler ce RDV</a></button>
            </h2>
